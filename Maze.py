...
class Agent:
    ...
    def random_q_table(self):
        self.q_table_iteration += 1
        if self.q_table_iteration == 6:
            self.q_table_iteration = 1
        abc = maze.LoadMaze("Maze" + str(self.q_table_iteration) + ".csv")
        self.last_known_maze = abc.copy()
        self.q_table_path = "q_table" + str(self.q_table_iteration) + ".csv"
    ...
    def monitor_route_finall(self, maze_dictionary):
        if self.last_known_maze is not None and maze_dictionary[self.state[1], self.state[0]] != self.last_known_maze[
            self.state[1], self.state[0]]:
            maze.reset_agent()
            self.q_table.clear()
    ...
    def start_escape(self, maze, maze_dictionary2):
        maze.reset_agent()

        while not maze.is_goal_reached():
            state = maze.get_state()
            action = self.choose_action(state)
            maze.move_agent(action, maze_dictionary2)
            next_state = maze.get_state()

            if time.time() - self.last_change_time >= 4:
                maze_dictionary2 = maze.LoadMaze("Maze" + str(random.randint(1, 5)) + ".csv")
                maze.DrawMaze(maze_dictionary2)
                window.update()
                self.last_change_time = time.time()
            ...
            self.learn(next_state, reward)
            self.monitor_route_finall(maze_dictionary2)
            maze.DrawAgent()
            window.update()
            time.sleep(0.1)
...
for _ in range(num_episodes):
    ...
    srednia = 0
    if _ > 5:
        last20 = agent.episode_rewards[-5:]
        srednia = sum(last20) / len(last20)
        if min(last20) >= srednia - 10 and max(last20) <= srednia + 10:
            agent.last_known_maze = maze_dictionary.copy()
            maze.maze_iteration += 1

            if maze.maze_iteration >= 6:
                break
...
agent.last_known_maze = None
maze_dictionary = maze.LoadMaze("Maze" + str(random.randint(1, 5)) + ".csv")
maze.DrawMaze(maze_dictionary)
window.update()
agent.q_table_iteration = 1
agent.q_table_path = "q_table" + str(agent.q_table_iteration) + ".csv"
agent.load_q_table_from_csv()
maze_dictionary_test = maze.LoadMaze("Maze" + str(agent.q_table_iteration) + ".csv")
agent.last_known_maze = maze_dictionary_test.copy()

agent.start_escape(maze, maze_dictionary)
